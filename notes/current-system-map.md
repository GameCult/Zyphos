...
The vault has three working layers.
...
`state/map.yaml` stores the current project map. `state/evidence.jsonl` stores durable research and design lessons. `state/branches.json` stores open design branches. `state/scratch.md` is disposable. `tools/setting_state.py` and `tools/prepare_compaction.py` provide status and compaction hygiene.
