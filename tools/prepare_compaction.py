...
PLAN_PATH = NOTES_DIR / "implementation-plan.md"
...
def add_agents_checks(findings: list[Finding]) -> None:
    ...
    if "prepare_compaction.py" in text:
        findings.append(Finding("ok", "AGENTS.md tells agents to use the compaction helper"))
    else:
        findings.append(Finding("error", "AGENTS.md does not mention tools/prepare_compaction.py"))
...
def render_report(findings: list[Finding], status: str, log: str, evidence: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "Eusocial Interbeing pre-compaction persistence check",
        f"Workspace: {ROOT}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit Eusocial Interbeing vault persistent state before imminent compaction."
    )
    parser.add_argument(
        "--strict",
        action="store_true",
